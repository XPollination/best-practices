...
RECOVERY=$(curl -s --max-time 10 -X POST "$BRAIN_URL" \
  ...
  -d "{\"prompt\": \"Recovery after context compaction for ${ROLE} agent. Role definition, current responsibilities, active task state, and key operational rules.\", \"agent_id\": \"${AGENT_ID}\", \"agent_name\": \"${AGENT_NAME}\", \"session_id\": \"${SESSION_ID}\"}" 2>/dev/null)

# Query brain for current task state
TASKS=$(curl -s --max-time 10 -X POST "$BRAIN_URL" \
  -H "Content-Type: application/json" \
  -d "{\"prompt\": \"Current task state, in-flight work, and recent decisions across all projects for ${ROLE} agent\", \"agent_id\": \"${AGENT_ID}\", \"agent_name\": \"${AGENT_NAME}\", \"session_id\": \"${SESSION_ID}\"}" 2>/dev/null)
